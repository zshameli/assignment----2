...
class TestEBookStore(unittest.TestCase):
    ...
    def test_add_ebook_to_catalog(self):
        ...
        self.assertEqual(self.ebook1.get_title(), "Python Programming")
        self.assertEqual(self.ebook1.get_price(), 29.99)

    def test_modify_ebook_details(self):
        """Test modifying details of an e-book"""
        self.ebook1.set_price(24.99)  # modifying price with setter
        self.assertEqual(self.ebook1.get_price(), 24.99)
    ...
    def test_add_customer_account(self):
        ...
        self.assertEqual(new_customer.get_name(), "Alice Smith")
        self.assertFalse(new_customer.get_is_loyalty_member())

    def test_modify_customer_account(self):
        """Test modifying customer account details"""
        self.customer.set_is_loyalty_member(False)
        self.assertFalse(self.customer.get_is_loyalty_member())

    def test_add_ebook_to_shopping_cart(self):
        """Test adding e-books to the shopping cart"""
        self.cart.add_item(self.ebook1, 2)
        items = self.cart.get_items()  # Assuming get_items() is defined in ShoppingCart
        self.assertEqual(len(items), 1)  # One item added
        self.assertEqual(items[0].get_quantity(), 2)

    def test_remove_item_from_cart(self):
        """Test removing items from the shopping cart"""
        self.cart.add_item(self.ebook1, 1)
        self.cart.remove_item(self.ebook1)  # Assuming remove_item is defined in ShoppingCart
    ...
    def test_apply_loyalty_discount(self):
        ...
        expected_total = self.ebook1.get_price() * 0.9  # 10% loyalty discount applied
    ...
    def test_bulk_purchase_discount(self):
        ...
        expected_total = 8 * self.ebook1.get_price() * 0.8  # 20% off bulk purchase
    ...
    def test_invoice_generation(self):
        ...
        self.assertIsInstance(self.order.get_invoice(), Invoice)
        self.assertAlmostEqual(self.order.get_invoice().get_invoice_total(), 69.98)

    def test_payment_processing(self):
        """Test payment processing for an order"""
        payment = Payment("2023-10-25", "Credit Card", 69.98)
        self.assertEqual(payment.get_amount(), 69.98)  # Assuming get_amount() is defined in Payment
        self.assertEqual(payment.get_payment_method(), "Credit Card")  # Assuming get_payment_method() is defined
    ...
    def test_cart_total_amount(self):
        ...
        self.cart.add_item(self.ebook1, 2)
        total_amount = self.cart.calculate_total()  # Assuming calculate_total() is defined in ShoppingCart
